		...
		if(strstr(PHP_VERSION, '7.'))
		{
    		...
    		if(empty($error)) {
     		...
     		if(empty($error)) {
                ...
                //循环添加数据
                if(file_exists(IA_ROOT . '/niushop_b2b2c.sql')){
                    ...
                    $sql = file_get_contents(IA_ROOT . '/niushop_b2b2c.sql');
                    $sql = str_replace("\r", "\n", $sql);
                    $sql = explode(";\n", $sql);
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" id="dbname" value="niushop_b2b2c">
					</div>
				</div>
			</div>
		</div>
...
// 	$themes = get_store_theme();
	...
	<div class="alert alert-success">
	    ...
	    <a target="_blank" class="btn btn-success" href="./admin">访问店铺后台</a>
		<a target="_blank" class="btn btn-success" href="./platform">访问平台后台</a>
	</div>
	<div class="form-group">
		<h5><strong></strong></h5>		
	</div>
