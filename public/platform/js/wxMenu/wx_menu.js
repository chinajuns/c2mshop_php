...
//网址验证
var httpRegular = "^(http|ftp)\://([a-zA-Z0-9\.\-]+(\:[a-zA-Z0-9\.&amp;%\$\-]+)*@)*((25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9])\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[0-9])|localhost|([a-zA-Z0-9\-]+\.)*[a-zA-Z0-9\-]+\.(com|edu|gov|int|mil|net|org|biz|arpa|info|name|pro|aero|coop|museum|[a-zA-Z]{2}))(\:[0-9]+)*(/($|[a-zA-Z0-9\.\,\?\'\\\+&amp;%\$#\=~_\-]+))*$";
...
$(function() {
	...
	$(".jsMenu").live("click",function(){
		 ...
		 * 当前菜单是否有图文消息
		 */
		setMediaDisplay($(this).attr("data-mediaid"));//设置图文消息列表是否显示,没有则清空图文消息

		//单图文、多图文消息
		if(parseInt($(this).attr("data-menu-type")) == 2 || parseInt($(this).attr("data-menu-type")) == 3){
		...
		var menu_event_url = menu_event_url = $(this).attr("data-menu-eventurl");//菜单url
	...
	$(".jsSubMenuInner").live("click",function(){
		 ...
		 * 当前菜单是否有图文消息
		 */
		setMediaDisplay($(this).find("a").attr("data-mediaid"));
		setRightMenuDetail();//每次点击一级菜单后，让右侧的编程内容还原（显示菜单内容，）

		//单图文、多图文消息
		if(parseInt($(this).find("a").attr("data-menu-type")) == 2 || parseInt($(this).find("a").attr("data-menu-type")) == 3){
			$(".js_radio_sendMsg").addClass("selected");
			$(".js_radio_url").removeClass("selected");
			$("#edit").show();
			$("#url").hide();
	...
	$("#pubBt").click(function(){
		if(IS_OK){
			$.ajax({
				url : PLATFORMMAIN+"/Wchat/updateMenuToWeixin",
				type : "post",
				success : function(res){
					showDialog(res.code,1);
				}
			})
		}else{
			showDialog("请先编辑完当前操作！！！", 0);
		}
	})
	...
	$(".jsmsgSenderDelBt").click(function(){
		...
		var obj = getCurrentMenu();
		//删除图文消息后，类型改为文本
		updateWeiXinMenuMessage(obj.attr("data-menuid"), 0, 1);
	})
...
function updateInfoNew(menu){
	...
	if(menu.pid == 0){
		...
		if(parseInt($("a[data-menuid='"+menu.pid+"']").attr("data-menu-type"))!= 1){
			...
			updateWeiXinMenuMessage(menu.pid, 0, 1);
		}
	}
}
...
function updateWeixinMenuEventType(menu_id,menu_event_type){
	$.ajax({
		url : PLATFORMMAIN + "/Wchat/updateWeixinMenuEventType",
		type : "post",
...
function addWeixinMenu(menu){
	$.ajax({
		url : PLATFORMMAIN + "/Wchat/addWeixinMenu",
		type : "post",
		data : { "menu" : menu },
...
function updateWeixinMenuName(menu_id, menu_name){
	$.ajax({
		url : PLATFORMMAIN + "/Wchat/updateWeixinMenuName",
		type : "post",
...
function updateWeixinMenuUrl(menu_id,menu_event_url){
	$.ajax({
		url : PLATFORMMAIN + "/Wchat/updateWeixinMenuUrl",
		type : "post",
...
function deleteWeixinMenu(menu_id){
	...
	$.ajax({
		url : PLATFORMMAIN + "/Wchat/deleteWeixinMenu",
		type : "post",
...
function updateWeiXinMenuMessage(menu_id, media_id, menu_event_type){
	$.ajax({
		url : PLATFORMMAIN + "/Wchat/updateWeiXinMenuMessage",
		type : "post",
...
function getWeixinMediaDetail(media_id,flag){
	$.ajax({
		...
		success : function(mediaObj){
			...
			if(flag){
				var obj = getCurrentMenu();
				updateWeiXinMenuMessage(obj.attr("data-menuid"), media_id, mediaObj.type);
			}
		}
	})
}
	...
	if(mediaObj.type == 2 || mediaObj.type == 3){
		...
		for(var i=0;i<list.length;i++){
			if(i == 0){
				...
				var style = "background-image:url('"+UPLOAD+"/"+list[i]["cover"]+"')";
				$(".cover_appmsg_item>div").attr("style",style);
				$(".cover_appmsg_item>div").text("");
			}else{
				html += '<div class="appmsg_item has_cover">';
				html += '<div class="appmsg_thumb_wrp" style="background-image:url('+UPLOAD+"/"+list[i]["cover"]+');"></div>';
				html += '<h4 class="appmsg_title js_title"><a href="'+list[i]["content_source_url"]+'" target="_blank">'+list[i]["title"]+'</a></h4>';
				//预览文章地址
				html += '<a href="javascript:showDialog(&#39;微信还没有给您生成呢，想啥呢？不能预览！！！&#39;);" class="edit_mask preview_mask js_preview" >';
				html += '<div class="edit_mask_content"><p style="color:#fff;">预览文章</p></div>';
				html += '<span class="vm_box"></span></a></div>';
			}
		}	
	}else{
		//单文本
...
function updateWeixinMenuSort(menu_id_arr,flag){
	$.ajax({
		url : PLATFORMMAIN + "/Wchat/updateWeixinMenuSort",
		type : "post",
