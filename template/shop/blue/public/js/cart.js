...
function select_cart_goods() {
	...
	// alert("J:"+j+",c:"+c+",sel_goods:"+sel_goods);
...
function add_num(cart_id, max_buy, goodsid, obj, stock) {
	...
	if (max_buy == 0) {// 不限购
		...
		if (count > 1 && count >= max_buy) {// 同样商品，不同SKU
			$.msg("该商品限购" + max_buy + "件");
			temp_num = max_buy;
			is_update = false;
		} else if (number < max_buy) {
			number++;
			temp_num = number;
		} else {
			$.msg("该商品限购" + max_buy + "件");
	...
	if (is_update) {
		...
		getCopeSum();// 刷新应付总额
		updateGoodsNumber(cart_id, temp_num);// 更新商品数量
	}
}
...
function minus_num(cart_id, max_buy, stock, min_buy) {
	...
	if (number > 1) {
		if(min_buy >= number){
			...
			$("#subtotal_" + cart_id).attr("data-total", total_price);
			getCopeSum();// 刷新应付总额
			updateGoodsNumber(cart_id, number);// 更新商品数量
		}	
	}
}
	...
	if (!r.test(number)) {
		...
		if (max_buy == 0) {// 不限购
			...
			if (count > 1 && count >= max_buy) {// 同样商品，不同SKU，只有限购的情况下，才需要判断count的数量
				temp_num = max_buy;
				$.msg("该商品限购" + temp_num + "件");
				is_update = false;
			} else if (number <= max_buy) {
				temp_num = number;// 正常情况
			} else {
				temp_num = max_buy;
				$.msg("该商品限购" + temp_num + "件");
				is_update = false;
