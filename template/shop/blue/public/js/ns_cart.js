							...
							$.ajax({
								...
								success : function(res){
									if(res == -1)
									{
										...
										//window.location.href = SHOPMAIN+ "/login/index";
									}else{
										$(".add-cart").removeClass("js-disabled");
										$(".js-buy-now").removeClass("js-disabled");
										//加入购物车
...
//				}, "json");
		...
		remove: function(cart_ids, callback) {
			...
			this.request = $.post(__URL(SHOPMAIN+'/cart/remove'), {
				cart_ids: cart_ids
			}, function(result) {
		...
		del: function(cart_ids, callback) {
			...
			this.request = $.post(__URL(SHOPMAIN+'/cart/delete'), {
				cart_ids: cart_ids
			}, function(result) {
		...
		changeNumber: function(sku_id, number, callback) {
			...
			this.request = $.post(__URL(SHOPMAIN+'/cart/change-number'), {
				sku_id: sku_id,
				number: number
			}, function(result) {
		...
		select: function(cart_ids, callback) {
			...
			this.request = $.post(__URL(SHOPMAIN+'/cart/select'), {
