...
class Promotion extends BaseController
{
    ...
    public function setstatus(){
        ...
        $mansong = new PromotionService();
        $res = $mansong -> setStatusPromotionMansong($mansong_id,$type);
        return AjaxReturn($res);
    }
    /**
     * 满减送详情
    ...
    public function promotioninfo(){
        $mansong_id = isset($_GET['mansong_id']) ? $_GET['mansong_id'] : 0;
        
        if($mansong_id == 0) {
            $this->error("没有获取到满减活动信息");
        }
        $mansong_service = new PromotionService();
        $detail = $mansong_service->getPromotionMansongDetail($mansong_id);
        if (empty($detail)) {
            $this->error("没有获取到满减活动信息");
        }
        // var_dump($detail);die;
        $this->assign("mansong", $detail);
        return view($this->style . "Promotion/mansonginfo");
    }
    ...
    public function setdiscountstatus(){
        ...
        $type = request()->post('type','');
        $mansong = new PromotionService();
        $res = $mansong -> setStatusPromotionDiscount($discount_id,$type);
        return AjaxReturn($res);
    }
    /**
     * 限时折扣详情
    ...
    public function discountinfo(){
        ...
        if($discount_id == 0) {
            $this->error("没有获取到限时折扣活动信息");
        }
        $account_service = new PromotionService();
        $detail = $account_service->getPromotionDiscountDetail($discount_id);
        if (empty($detail)) {
            $this->error("没有获取到限时折扣信息");
        }
        // var_dump($detail);die;
