...
class Account extends BaseController
{
    ...
    public function shopAccountList(){
        if(request()->isAjax()){
            ...
            return view($this->style . "Account/shopAccountList");
        }
    }
    /**
     * 商品销售
     */
    public function shopOrderGoodsAccountRecordsList(){
        if(request()->isAjax()){
    
            $pageindex = isset($_POST['pageIndex']) ? $_POST['pageIndex'] : 1;
            $start_date = !empty($_POST['start_date']) ? $_POST['start_date'] : '';
            $end_date = !empty($_POST['end_date']) ? $_POST['end_date'] : '';
            $is_refund = !empty($_POST['is_refund']) ? $_POST['is_refund'] : '';
            $is_issue = !empty($_POST['is_issue']) ? $_POST['is_issue'] : '';
            $goods_id = !empty($_POST['goods_id']) ? $_POST['goods_id'] : 0;
            $condition = array();
            if($is_issue != 3){
                $condition["nsoar.is_issue"] = $is_issue;
            }
            if($is_refund != 3){
                $condition["nsoar.is_refund"] = $is_refund;
            }
            if($start_date != ""){
                $condition["nsoar.create_time"][] =[">",$start_date];
                $count_condition["nsoar.create_time"][] =[">",$start_date];
            }
            if($end_date != ""){
                $condition["nsoar.create_time"][] =["<",$end_date];
                $count_condition["nsoar.create_time"][] =["<",$end_date];
            }
            if($goods_id != 0){
                $condition["nog.goods_id"] = $goods_id;
                $count_condition["nog.goods_id"] = $goods_id;
            }
//             $condition["nsoar.order_id"] = [">",0];
            $shop = new Shop();
            //var_dump($commission_calculate);
            $list = $shop->getshopOrderAccountRecordsList($pageindex,PAGESIZE,$condition, 'create_time desc');
            $count = $shop->getShopGoodsSales($count_condition);
            return ["list"=>$list,"count"=>$count];
        }else{
            $goods_id = isset($_GET["goods_id"]) ? $_GET["goods_id"] : 0;
            $this->assign("goods_id",$goods_id);
            return view($this->style . "Account/shopOrderGoodsAccountRecordsList");
...
//             $condition=array();
            ...
            return view($this->style . "Account/shopAccountRecordsList");
        }
    }
    ...
    public function platformaccountrecordscountlist()
    {
        if (request()->isAjax()) {
            ...
            $start_date = request()->post('start-date','2010-1-1');
            $end_date = request()->post('end_date','2099-1-1');
            $condition["create_time"] = [
                [
                    ">",
                    $start_date
            ...
            return view($this->style . "Account/platformAccountRecordsCountList");
        }
    }
     ...
     * 查询一段时间内的总下单量及下单金额
     *
     * @return multitype:\app\platform\controller\Ambigous Ambigous <\app\platform\controller\Ambigous, number, \data\service\niushop\unknown, \data\service\niushop\Order\unknown, unknown>
     */
