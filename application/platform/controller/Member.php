   ...
   /**
    ...
    public function feedbackList(){
        if (request()->isAjax()) { 
            ...
            $member = new MemberService();
            $res = $member->getFeedbackList($page_index, 20,'','create_time desc');
            foreach($res['data'] as $k=>$v){
                ...
                $v[nick_name] = $userinfo['nick_name'];
     ...
     * 回复
     */
    public function replyOpinion(){
        if (request()->isAjax()) {
            $id = request()->post("id", '');
            $reply = request()->post("reply", '');
            $member = new MemberService();
            $retval = $member->replyFeedback($id,$reply);
            return AjaxReturn($retval);
        }
    }
