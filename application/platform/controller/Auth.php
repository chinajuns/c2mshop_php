...
class Auth extends BaseController
{
    ...
    public function addAuthGroup()
    {
        if (request()->isAjax()) {
            ...
            $retval = $this->auth_group->addSystemUserGroup(0, $group_name, false, $module_id_array, '');
            return AjaxReturn($retval);
        }
        // 获取权限
        $p = $this->getPermissionList();
        $this->assign("list", $p);
        return view($this->style . "Auth/addAuthGroup");
    }
    ...
    public function updateAuthGroup()
    {
        if (request()->isAjax()) {
            ...
            $group_name = isset($_POST['group_name']) ? $_POST['group_name'] : '';
            $module_id_array = isset($_POST['select_box']) ? $_POST['select_box'] : '';
            $res = $this->auth_group->updateSystemUserGroup($group_id, $group_name, 1, false, $module_id_array, '');
        ...
        $p = $this->getPermissionList();
        $this->assign("list", $p);
        return view($this->style . "Auth/updateAuthGroup");
    }
    /**
     * 查询权限列表
     *
    ...
    public function getPermissionList()
    {
        ...
        for ($i = 0; $i < count($permissionList); $i ++) {
            $per = $permissionList[$i];
            // if($per["parentId"] == null && $per["name"] != null){
            // $firstArray[] = $per;
            // }
            if ($per["pid"] == 0 && $per["module_name"] != null) {
                $firstArray[] = $per;
