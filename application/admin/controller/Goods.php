...
//                 'ng.sort' => 'desc',
    ...
    public function addGoods()
    {
        ...
        $supplier_list = $supplier->getSupplierList(1,0, ['shop_id' => $this->instance_id]);
        $this->assign("supplier_list", $supplier_list['data']);
...
$product['goods_attribute_id'], $product['goods_attribute'], $product['goods_spec_format'], $product['goods_weight'], $product['goods_volume'], $product['shipping_fee_type'], $product['categoryExtendId'], $product["sku_picture_vlaues"]);
    ...
    public function getSearchGoodsList()
    {
        ...
        if(!empty($condition))
        {
            $condition_arr['goods_name'] = array('like', '%'.$condition.'%');
        }
        $condition_arr['shop_id'] = $this->instance_id;
        
        $goods_detail = new GoodsService();
        $result = $goods_detail->getSearchGoodsList($page_index, $page_size, $condition_arr);
        return $result;
    }
    ...
    public function goodsSpecList()
    {
        ...
        if (request()->isAjax()) {
            ...
            $list = $goods->getGoodsSpecList($page_index, $page_size, $condition, 'sort asc, create_time desc');
            return $list;
        }
    ...
    public function addGoodsSpec()
    {
        ...
        //查询商品类型
        $goods_attribute_list = $goods->getAttributeServiceList(1, 0, ['is_use' => 1], '');
    ...
    public function updateGoodsSpec()
    {
        ...
        if (request()->isAjax()) {
            ...
            $spec_value_str = request()->post('spec_value_str', '');
            $is_screen = request()->post('is_screen', 0);
            $res = $goods->updateGoodsSpecService($spec_id, $this->instance_id, $spec_name, $show_type, $is_visible, $sort, $spec_value_str, $attr_id,$is_screen);
            return AjaxReturn($res);
        }
