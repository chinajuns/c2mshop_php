...
class Member extends BaseController
{
    ...
    public function index()
    {
        ...
        switch (NS_VERSION) {
            case NS_VAR_B2B2C:
                $retval = $this->memberIndex($shop_id); // 单店B2C版
                break;
            case NS_VAR_B2B2C_FX:
                $retval = $this->memberIndexFx($shop_id); // 单店B2C分销版
                break;
        }
        return $retval;
    }
     ...
     * 单店B2C版
     */
    public function memberIndex($shop_id)
    {
     ...
     * @return Ambigous <\think\response\View, \think\response\$this, \think\response\View>
     */
    public function memberIndexFx($shop_id)
    {
        $member = new MemberService();
    ...
    {
        $uid = $this->user->getSessionUid();
        $instance_id = request()->get('shop_id', 0);
        if ($instance_id == 0) {
    ...
    public function shopCollection()
    {
        if (request()->isAjax()) {
            ...
            $goods_collection_list = $member->getMemberShopsFavoritesList($page, PAGESIZE, $condition, "fav_time desc");
            foreach ($goods_collection_list['data'] as $k => $v) {
                $v['fav_time'] = date("Y-m-d H:i:s", $v['fav_time']);
            }
        ...
        return view($this->style . 'Member/shopCollection');
    }
