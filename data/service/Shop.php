...
use data\model\NsArrivalGuideModel;
...
class Shop extends BaseService implements IShop
{
    ...
    public function updateShopAd($id, $ad_image, $link_url, $sort, $type, $background)
    {
        ...
        $data['background'] = $background;
        $shop_ad = new NsShopAdModel();
        $shop_ad_info = $shop_ad->getInfo(['id' => $id], 'shop_id');
        if(empty($shop_ad_info) || $shop_ad_info['shop_id'] != $this->instance_id)
        {
            return NO_AITHORITY;
        }
        $res = $shop_ad->save($data, [
    ...
    */
     ...
     */
    public function updateShopConfigByPlatform($shop_id, $shop_name, $shop_group_id, $shop_platform_commission_rate, $shop_type, $shop_credit, $shop_desccredit, $shop_servicecredit, $shop_deliverycredit, $store_qtian, $shop_zhping, $shop_erxiaoshi, $shop_tuihuo, $shop_shiyong, $shop_shiti, $shop_xiaoxie, $shop_huodaofk, $shop_state, $shop_close_info)
    {
        $data = array(
            'shop_name' => $shop_name,
            'shop_group_id' => $shop_group_id,
     ...
     */
    public function updateShopShareCinfig($shop_id, $goods_param_1, $goods_param_2, $shop_param_1, $shop_param_2, $shop_param_3, $qrcode_param_1, $qrcode_param_2,$platform_param_1,$platform_param_2,$platform_param_3)
    {
        ...
        $data = array(
            ...
            'platform_param_1' => $platform_param_1,
            'platform_param_2' => $platform_param_2,
            'platform_param_3' => $platform_param_3
        );
    ...
    public function getShopAccountCountList($page_index, $page_size = 0, $condition = '', $order = '')
    {
        ...
        foreach ($list["data"] as $k => $v) {
            ...
            $list["data"][$k]["shop_canwithdraw"] = $v['shop_proceeds']-$v['shop_withdraw'];
        }
        return $list;
    }
    ...
    {
        $shop_order_return_model = new NsShopOrderReturnModel();
     ...
     */
    public function addPlatformShop($user_account, $user_pwd, $shop_name, $shop_group_id, $shop_type){
        ...
        try {
            ...
            $res = $member_service->addMember($user_account, $user_pwd, '', 0, 1, '', $member_level);
            if($res >0){
                $res = $this->addshop($shop_name, $shop_type, $res, $shop_group_id, '', '', '', '', '', '');
            }   
            $shop_model->commit();
            return $res;
        }catch (\Exception $e){
     ...
     * 指南
     */
    public function getGuideList($page_index = 1, $page_size = 0, $condition = '', $order = '')
    {
        $shop = new NsArrivalGuideModel();
        $list = $shop->pageQuery($page_index, $page_size, $condition, 'sort ASC', '*');
        return $list;
    }
    /*
     * 添加指南
     * (non-PHPdoc)
     */
    public function addGuide($title,$sort,$image,$content)
    {
        // TODO Auto-generated method stub
        $shop = new NsArrivalGuideModel();
        $data = array(
     ...
     * (non-PHPdoc)
     */
    public function updateGuide($guide_id,$title,$sort,$image,$content)
    {
        // TODO Auto-generated method stub
        $shop = new NsArrivalGuideModel();
        $data = array(
        ...
        $retval = $shop->save($data, ['guide_id' => $guide_id]);
        return $retval;
    }
    public function getGuideDetail($guide_id){
        $shop = new NsArrivalGuideModel();
        $retval = $shop->get($guide_id);
        return $retval;
    }
