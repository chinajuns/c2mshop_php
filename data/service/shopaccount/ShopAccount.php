...
class ShopAccount extends BaseService
{
    ...
    public function addShopAccountProfitRecords($serial_no, $shop_id, $money, $account_type, $type_alis_id, $remark)
    {
        ...
        if (empty($records_list)) {
            ...
            try {
                ...
                $shop_account_profit_model = new NsShopAccountProfitRecordsModel();
                $records_id = $shop_account_profit_model->save($data);
                // 更新营业总额字段
                $this->updateShopAccountProfit($shop_id, $money);
                $model->commit();
            } catch (\Exception $e) {
                $model->rollback();
                Log::write("错误addShopAccountProfitRecords:".$e->getMessage());
                return $e->getMessage();
            }
        }
    }
    ...
    public function addShopAccountMoneyRecords($serial_no, $shop_id, $money, $account_type, $type_alis_id, $remark)
    {
        ...
        if (empty($records_list)) {
            ...
            try {
                ...
                $model->rollback();
                Log::write("错误addShopAccountMoneyRecords".$e->getMessage());
                return $e->getMessage();
            }
        }
    }
    ...
    public function addShopAccountProceedsRecords($serial_no, $shop_id, $money, $account_type, $type_alis_id, $remark)
    {
        ...
        if (empty($records_list)) {
            ...
            try {
                ...
                $model->rollback();
                Log::write("错误addShopAccountProceedsRecords:".$e->getMessage());
                return $e->getMessage();
            }
        }
    }
    ...
    private function addShopAccountReturnRecords($serial_no, $shop_id, $money, $account_type, $type_alis_id, $remark)
    {
        ...
        if (empty($records_list)) {
            ...
            try {
                ...
                $model->rollback();
                Log::write("错误addShopAccountReturnRecords:".$e->getMessage());
                return $e->getMessage();
            }
        }
    }
    ...
    public function addShopAccountWithdrawRecords($serial_no, $shop_id, $money, $account_type, $type_alis_id, $remark)
    {
        ...
        if (empty($records_list)) {
            ...
            try {
                ...
                Log::write("错误addShopAccountWithdrawRecords:".$e->getMessage());
                $model->rollback();
                return $e->getMessage();
            }
        }
    ...
    public function updateShopOrderAccountRecord($order_id)
    {
        ...
        if (! empty($order_return_obj)) {
            ...
            try {
                $order_return_money = $order_return_obj["platform_money"];
                $real_pay = $order_return_obj["order_pay_money"];
                $shop_id = $order_return_obj["shop_id"];
                if ($order_return_money > 0) {
                    $this->addShopAccountReturnRecords(getSerialNo(), $shop_id, $order_return_money, 1, $order_id, "店铺订单完成，订单金额：" . $real_pay . "元, 平台抽取" . $order_return_money . "。");
                }
                ...
                Log::write("错误updateShopOrderAccountRecord:".$e->getMessage());
