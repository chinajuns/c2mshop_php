...
use think\Session;
use think\Log;
use data\model\NsMemberFeedbackModel;
...
class Member extends User implements IMember
{
        ...
        if (! empty($pay_info)) {
            ...
            if ($pay_status == 1) {
                ...
                if (! empty($racharge_obj)) {
                    ...
                    runhook("Notify", "rechargeSuccessBusiness", ["shop_id"=>0,"out_trade_no"=>$out_trade_no,"uid"=>$racharge_obj["uid"]]);//用户余额充值成功商家提醒
                    runhook("Notify", "rechargeSuccessUser", ["shop_id"=>0,"out_trade_no"=>$out_trade_no,"uid"=>$racharge_obj["uid"]]);//用户余额充值成功用户提醒
                }
            }
        }
    }
    ...
    private function verifyValue($user_name, $password, $reg_type = "plain")
    {
        ...
        if ($reg_config["is_register"] != 1) {
            return array(false, REGISTER_CONFIG_OFF);
        }
        if ($reg_type == "mobile") {
            if (stristr($reg_config["register_info"], "mobile") === false) {
                return array(false, REGISTER_MOBILE_CONFIG_OFF);
            }
        } else 
            if ($reg_type == "email") {
                if (stristr($reg_config["register_info"], "email") === false) {
                    return array(false, REGISTER_EMAIL_CONFIG_OFF);
                }
            } else {
                if (stristr($reg_config["register_info"], "plain") === false) {
                    return array(false, REGISTER_PLAIN_CONFIG_OFF);
                }
                if (trim($user_name) == "") {
                    return array(false, REGISTER_USERNAME_ERROR);
                }
        ...
        if ($password_len == 0) {
            return array(false,REGISTER_PASSWORD_ERROR);
        }
    ...
    public function judgeUserNameIsExistence($user_name){
        $user = new UserModel();
        $res = $user -> getCount(["user_name"=>$user_name]);
        if($res > 0){
            return true;
        }else{
            return false;
    ...
    public function getFeedbackList($page_index = 1, $page_size = 0, $condition = '', $order = ''){
        $member = new NsMemberFeedbackModel();
        $list = $member->pageQuery($page_index, $page_size, $condition, $order, '*');
        return $list;
    }
    /*
     * 删除意见
     ...
     * 回复
     */
    public function replyFeedback($id,$reply){
        $member = new NsMemberFeedbackModel();
        $data = array(
            'reply'      => $reply,
            'reply_time' => time(),
