...
//                     }
    ...
    public function getRecommendGoodsList($shop_id,$show_num = 4)
    {
        ...
        $group_list = $group->getQuery(["shop_id"=>$shop_id], "*", "sort desc");
        foreach($group_list as $k=>$v){
            $group_goods_list = array();
            $goods_list = $goods->getGoodsViewList(1, $show_num, "FIND_IN_SET(".$v["group_id"].",ng.group_id_array) AND ng.state = 1", "ng.sort desc");
            //var_dump($goods_list);
            if(!empty($goods_list["data"])){
                foreach($goods_list["data"] as $t=>$m){
                    $is_exist = true;
