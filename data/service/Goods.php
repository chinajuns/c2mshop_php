...
class Goods extends BaseService implements IGoods
{
     ...
     */
    public function updateGoodsSpecService($spec_id, $shop_id, $spec_name, $show_type, $is_visible, $sort, $spec_value_str, $attr_id=0, $is_screen)
    {
        $goods_spec = new NsGoodsSpecModel();
        $goods_spec->startTrans();
        try {
    ...
    public function getGoodsAttrSpecQuery($condition)
    {
        ...
        if($model == 'admin')
        {
            $condition_ext = array(
                'shop_id' => $this->instance_id,
                'is_platform' => 0,
                'goods_attr_id' => $condition["attr_id"]
            );
            $ext_query = $this->getGoodsSpecQuery($condition_ext);
            if(!empty($ext_query))
            {
                $spec_list = array_merge($spec_list, $ext_query);
            }
        }
